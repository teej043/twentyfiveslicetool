...
if (loaded_sprite != -1 && !preview_mode) {
    ...
    var sprite_center_x = image_x + (sprite_width_val/2) * image_scale;
    var middle_left_x = sprite_center_x - (top_bot_width/2) * image_scale;
    var middle_right_x = sprite_center_x + (top_bot_width/2) * image_scale;
    ...
    // Horizontal lines
    var corner_top_y = image_y + corner_height * image_scale;  // Blue line top
    var sprite_center_y = image_y + (sprite_height_val/2) * image_scale;
    var middle_top_y = sprite_center_y - (sides_height/2) * image_scale;
    var middle_bottom_y = sprite_center_y + (sides_height/2) * image_scale;
    var corner_bottom_y = image_y + (sprite_height_val - corner_height) * image_scale;
    
    // Draw horizontal lines
    draw_line(image_x, corner_top_y, image_x + img_w, corner_top_y);  // Top blue
    draw_line(image_x, middle_top_y, image_x + img_w, middle_top_y);  // Top red
    draw_line(image_x, middle_bottom_y, image_x + img_w, middle_bottom_y);  // Bottom red
    draw_line(image_x, corner_bottom_y, image_x + img_w, corner_bottom_y);  // Bottom blue
    
    // Draw grid line handles
    var handle_size = 4;
    var large_handle_size = 6;  // Larger size for the sides height handle
    ...
    var handle_x = image_x + handle_drag_pos_x * image_scale;
    draw_rectangle(handle_x - large_handle_size, image_y + img_h/2 - large_handle_size, 
                  handle_x + large_handle_size, image_y + img_h/2 + large_handle_size, false);
    
    // Single horizontal green handle for sides_height (larger)
    var handle_y = image_y + handle_drag_pos_y * image_scale;
    draw_rectangle(image_x + img_w/2 - large_handle_size, handle_y - large_handle_size, 
                  image_x + img_w/2 + large_handle_size, handle_y + large_handle_size, false);
}
...
if (preview_mode && loaded_sprite != -1) {
    ...
    // Add label
    draw_set_color(c_black);
    draw_text(preview_x, preview_y - 40, "25-Slice Preview" + (preview_debug_view ? " (Debug View)" : ""));
}
